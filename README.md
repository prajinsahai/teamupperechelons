...
| **AI Actuary recommendation** | A concise 3-sentence recommendation on reserve adequacy and reinsurance protection | Claude (`claude-opus-5`) via a tool call that reads the engine + model outputs |
...
data/claims.csv ──► Actuarial engine (pure Python)  ──► IBNR, reinsurance recovery, net exposure
                ──► ML models (scikit-learn, joblib) ──► severity prediction, anomaly flags
                                       │
                                       ▼
                        Claude via langchain-anthropic (one tool, manual loop)
                                       │
                                       ▼
...
- **LLM:** one Claude model through `langchain-anthropic`, tool calling only
...
src/
  ...
  llm/
    ...
    agent.py                 ChatAnthropic + one tool + manual tool loop
  data/
